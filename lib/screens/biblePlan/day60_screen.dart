...
class _Day60ScreenState extends State<Day60Screen>
  ...
  void _scrollToCurrentDay() async {
    ...
    if (!planProvider.hasPlan(PlanType.day60)) return;

    final nextDay = planProvider.getNextDay(PlanType.day60);
    if (nextDay <= 1) return; // Day 1이면 스크롤 불필요

    // ScrollController가 완전히 연결될 때까지 대기
    await Future.delayed(const Duration(milliseconds: 300));

    if (!mounted || !_scrollController.hasClients) return;

    // 각 아이템의 대략적인 높이 (ListTile 높이 + Divider)
  ...
  Future<void> _showResetConfirmDialog() async {
    ...
    final progressPercent = planProvider.getProgressPercent(PlanType.day60);
    final completedDays = planProvider.getCompletedDaysCount(PlanType.day60);
    final totalDays = planProvider.getTotalDays(PlanType.day60);
  ...
  Future<void> _resetPlan() async {
    ...
    // 알림 취소 (Day60 플랜의 알림 ID는 60)
    await NotificationService().cancelNotification(60);

    // 플랜 삭제
    await planProvider.deletePlan(PlanType.day60);
  ...
  Future<void> _showNotificationSettingDialog() async {
    final planProvider = context.read<PlanProgressProvider>();
    final plan = planProvider.getPlan(PlanType.day60);
    ...
    final daysKey = 'notification_days_60';
                ...
                builder: (context, planProvider, _) {
                  return ListView.separated(
                    ...
                    itemBuilder: (_, idx) {
                      ...
                      // 읽음 처리 및 접근 가능 여부 확인
                      final isCompleted =
                          planProvider.isDayCompleted(PlanType.day60, dayNum);
                      final canAccess =
                          planProvider.canAccessDay(PlanType.day60, dayNum);
