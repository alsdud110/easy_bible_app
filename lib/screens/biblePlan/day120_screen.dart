...
class Day120Screen extends StatefulWidget {
  ...
  @override
  State<Day120Screen> createState() => _Day120ScreenState();
}

class _Day120ScreenState extends State<Day120Screen>
  ...
  void _scrollToCurrentDay() async {
    ...
    if (!planProvider.hasPlan(PlanType.day120)) return;

    final nextDay = planProvider.getNextDay(PlanType.day120);
    if (nextDay <= 1) return;
  ...
  Future<void> _showResetConfirmDialog() async {
    ...
    final progressPercent = planProvider.getProgressPercent(PlanType.day120);
    final completedDays = planProvider.getCompletedDaysCount(PlanType.day120);
    final totalDays = planProvider.getTotalDays(PlanType.day120);
  ...
  Future<void> _resetPlan() async {
    ...
    // 알림 취소 (Day120 플랜의 알림 ID는 120)
    await NotificationService().cancelNotification(120);

    // 플랜 삭제
    await planProvider.deletePlan(PlanType.day120);
  ...
  Future<void> _showNotificationSettingDialog() async {
    final planProvider = context.read<PlanProgressProvider>();
    final plan = planProvider.getPlan(PlanType.day120);
                ...
                builder: (context, planProvider, _) {
                  return ListView.separated(
                    ...
                    itemBuilder: (_, idx) {
                      ...
                      // 읽음 처리 및 접근 가능 여부 확인
                      final isCompleted = planProvider.isDayCompleted(PlanType.day120, dayNum);
                      final canAccess = planProvider.canAccessDay(PlanType.day120, dayNum);
